...
BS.Collection = BS.Collection || {
  ...
  buildSearchUrl: function(page) {
    ...
    var url = '/search?q=filter=' + encodeURIComponent(q) + '&view=filter&page=' + (page || 1);
  ...
  buildCollectionUrl: function(page) {
    ...
    var sort = BS.Collection.getSortVal();
    var url = collUrl + '?view=pagination&page=' + (page || 1);
  ...
  updateCount: function(total, useSearch) {
    ...
    if (!hasFilter && collCount) {
      countEl.textContent = collCount.textContent.replace(/[()]/g, '').trim() + ' kết quả';
    }
  },
  ...
  updateHeading: function(title, count) {
    ...
    if (filterCountEl && count != null) filterCountEl.textContent = count + ' kết quả';
  },
  ...
  renderActiveTags: function() {
    ...
    if (countEl && tags.length) countEl.textContent = tags.length + ' bộ lọc';
  },
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (descBtn && descInner) {
    descBtn.addEventListener('click', function() {
      ...
      descBtn.textContent = expanded ? 'THU GỌN' : 'ĐỌC THÊM';
