...
(function ($) {
  ...
  buyXgetY = {
    ...
    renderGiftItem(gift, is_xy_first, productTitle) {
	  const name = gift.title ? `${gift.product_name} - ${gift.title}` : gift.product_name
    console.log(gift)
      const image =
        gift.product_images.length > 0
          ? `<img loading="lazy" width="60" src="${gift.product_images[0]}" alt="${name}"/>`
          : `<img class="img_empty" src="${img_empty}" alt="${img_empty}"/>`;
      const input = `<input class="radio-product form-radio"
					 data-applyvariant=${gift.is_apply_variant}
					data-id="${gift.variant_id}" 
					data-percent="${gift.percent}" 
					data-countbuy="${gift.quantity}"
					data-countgift="${gift.apply_quantity}"
					${is_xy_first == 0 ? 'checked="checked"' : ""}
					type="radio" name="group-product-sale"/>
	`;
      const discount =
        gift.percent != null
          ? `${gift.percent}%`
          : Haravan.formatMoney(gift.amount * 100, formatMoney);
      const title = `<span class="price font-semibold">
		  ${
        gift.type == 3 || gift.percent == 100
          ? `Tặng ${gift.apply_quantity}`
          : `Giảm ${discount}`
      }  </span> <span> sản phẩm  <a href="${gift.product_url}" target="_blank" title="${
        name
      }">${name}</a>
		</span>
		<p class="bxy__tt2">  Khi mua  <span  class="font-semibold text-error"> ${
      gift.quantity
    } </span> <span>${productTitle}</span></p>
		
		`;
      return `<label class="buyxgety_item">
					<div class="buyxgety_checkbox">
						${input}
					</div>
					<div class="buyxgety_image">
							${image}
					</div>
					<div class="buyxgety_item_title">
						${title}
					</div>
			</label>
		`;
    ...
    getPromotionRecommended: function (id, title) {
      // truyền id, tên sản phẩm chính lấy từ product
      buyXgetY.check_getRecommendeds = false;
      var self = this;
      var img_empty = "{{settings.appbuyxgety_icon}}";
      $.get(
        "https://buyxgety-omni.haravan.com/js/recommendeds?product_id=" + id
      )
        .done(function (result) {
          ...
          if (result.recommendeds.length > 0) {
            ...
            var checkAvail = null;
            var strCheckAvailable =
              "(id:product=" + checkAvailable.join(")||(id:product=") + ")";
            var imgFinal = "";
            $.get(
              "/search?q=filter=((" +
                strCheckAvailable +
                ")&&(available:product=true))&view=datacombo"
            ).done((res) => {
              let gifts = JSON.parse(res);
              if ($.isEmptyObject(gifts)) {
                $("#buyxgety-program").html(
                  "Xin lỗi! Các sản phẩm quà tặng đã hết. Hẹn bạn vào lần khuyến mãi tới nhé!"
                );
              } else {
                var is_xy_first = 0;
                ...
                $.each(result.recommendeds, function (i, v) {
                  ...
                  if (gifts.hasOwnProperty(v.product_id)) {
                    if (gifts[v.product_id].available) {
                      var html2 = "";

                      if (v.is_apply_variant == false) {
						let variantAvailable = gifts[
                            v.product_id
                          ].variants.find(
                            (v) => v.available
                          );
						  v.variant_id = variantAvailable.id
                        html = self.renderGiftItem(v, is_xy_first,title);
                        $(
                          "#detail-product #buyxgety-product-list > .buyxgety_content > .buyxgety_lists"
                        ).append(html);
                      } else {
                        $.each(v.apply_variants, function (i2, v2) {
                          var checkvariant = gifts[
                            v.product_id
                          ].variants.find(
                            (v) => v.id == v2.variant_id
                          ).available;
                          if (checkvariant) {
							...
							var html = self.renderGiftItem(tempV, is_xy_first, title)
                            $(
                              "#detail-product #buyxgety-product-list > .buyxgety_content > .buyxgety_lists"
                            ).append(html);
                            is_xy_first++;
                ...
                if (
                  count_unavailable > 0 &&
                  count_unavailable == result.recommendeds.length
                ) {
                  $(
                    "#detail-product .buyxgety-product-list #buyxgety-program"
                  ).html(
                    "Xin lỗi! Các sản phẩm quà tặng đã hết. Hẹn bạn vào lần khuyến mãi tới nhé!"
                  );
                } else {
                var htmlFinal = `<label class="buyxgety_item specOption">
                  <div class="buyxgety_checkbox"><input class="radio-product not_gift" type="radio" name="group-product-sale"></div>
                  <div class="buyxgety_item_title">Chỉ mua <span >${title}</span></div>
                </label>`;
              ...
              $(
                "#add-to-cart,#add-to-cartbottom,#buy-now,#add-to-cart-2,#buy-now-2"
              ).removeClass("loading");
            });
          } else {
    ...
    getQuickPromotionRecommended: function (id, title) {
      // truyền id, tên sản phẩm chính lấy từ product
      buyXgetY.check_getRecommendeds = false;
      var self = this;
      $.get(
          ...
          if (result.recommendeds.length > 0) {
            ...
            $.get(
              "/search?q=filter=((" +
                strCheckAvailable +
                ")&&(available:product=true))&view=datacombo"
            ).done(function (checkGift) {
              checkAvail = JSON.parse(checkGift);
              if ($.isEmptyObject(checkAvail)) {
                $(".q-selector-buyxgety #buyxgety-program").html(
                  "Xin lỗi! Các sản phẩm quà tặng đã hết. Hẹn bạn vào lần khuyến mãi tới nhé!"
                );
              } else {
                var is_xy_first = 0;
                ...
                $.each(result.recommendeds, function (i, v) {
                  if (checkAvail.hasOwnProperty(v.product_id)) {
                    if (checkAvail[v.product_id].available) {
                      ...
                      if (v.is_apply_variant == false) {
                        let html = `
      						<div class="buyxgety_item">
      							<div class="buyxgety_checkbox">
      								<input class="radio-product" data-applyvariant="${
                        v.is_apply_variant
                      }" data-id="${v.variant_ids[0]}" data-percent="${
                          v.percent
                        }" data-countbuy="${v.quantity}" data-countgift="${
                          v.apply_quantity
                        }" ${
                          is_xy_first == 0 ? 'checked="checked"' : ""
                        } type="radio" name="group-product-sale">
      							...
      							<div class="buyxgety_image">
      								<a href="/products/${v.product_handle}" target="_blank">
      									<img width="60" src="${v.product_images[0]}" alt="item 1"/>
      								</a>
      							</div>
      							<div class="buyxgety_item_title">
      								<p class="bxy__tt1">
      									${
                          v.type == 3 || v.percent == 100
                            ? `Tặng ${v.apply_quantity}`
                            : `Giảm ${
                                v.percent != null
                                  ? v.percent + "%"
                                  : Haravan.formatMoney(v.amount * 100, "")
                              }`
                        } sản phẩm <a href="/products/${
                          v.product_handle
                        }" target="_blank">${v.product_name}.</a>
      								</p>
      								<p class="bxy__tt2">
      									Khi mua <strong>${v.quantity}</strong><span> ${title}</span>
      								</p>
      							</div>
						</div>
					`;
                        htmlQvApp += html;
                        $(
                          ".q-selector-buyxgety #buyxgety-product-list > .buyxgety_content > .buyxgety_lists"
                        ).append(html);
                      } else {
                        $.each(v.apply_variants, function (i2, v2) {
                          let checkvariant = checkAvail[
                            v.product_id
                          ].variants.find(
                            (v) => v.id == v2.variant_id
                          ).available;
                          if (checkvariant) {
                            let title_variant = v2.title.replace("/", "-");
                            let html = `
								<div class="buyxgety_item">
									<div class="buyxgety_checkbox">
										<input class="radio-product" data-applyvariant="${
                      v.is_apply_variant
                    }" data-id="${v2.variant_id}" data-percent="${
                              v2.percent
                            }" data-countbuy="${v2.quantity}" data-countgift="${
                              v2.apply_quantity
                            }" ${
                              is_xy_first == 0 ? 'checked="checked"' : ""
                            } type="radio" name="group-product-sale">
									...
									<div class="buyxgety_image">
										<a href="/products/${v.product_handle}?variant=${
                              v2.variant_id
                            }" target="_blank">
											${
                        v.product_images.length > 0
                          ? `<img width="60" src="${v.product_images[0]}" alt="item 1"/>`
                          : `<img width="60" class="img_empty" src="${img_empty}" alt="item 1"/>`
                      }
										</a>
									</div>
									<div class="buyxgety_item_title">
										<p style="margin-bottom:0px;">
											Khi mua <strong style="color:#f51e1e;">${
                        v2.quantity
                      }</strong><span style="color:#315399;"> ${title}</span><br> 
											${
                        v2.type == 3 || v2.percent == 100
                          ? `Tặng <strong style="color:#f51e1e;">${v2.apply_quantity}</strong><a href="/products/${v.product_handle}" target="_blank" style="color:#315399;"> ${v.product_name} - ${v2.title}</a>.`
                          : `Giảm <strong style="color:#f51e1e;">${
                              v2.percent != null
                                ? v2.percent + "%"
                                : Haravan.formatMoney(
                                    v2.amount * 100,
                            ...
                            }</strong> sản phẩm <a href="/products/${
                              v.product_handle
                            }?variant=${
                              v2.variant_id
                            }" target="_blank" style="color:#315399;">${
                              v.product_name
                            } - ${v2.title}.</a>`
                      }
										</p>
                ...
                if (
                  ...
                  $(
                    ".q-selector-buyxgety .buyxgety-product-list #buyxgety-program"
                  ).html(
                    "Xin lỗi! Các sản phẩm quà tặng đã hết. Hẹn bạn vào lần khuyến mãi tới nhé!"
                  );
                } else {
                  ...
                  htmlFinal +=
                    '<div class="buyxgety_checkbox"><input class="radio-product not_gift" type="radio" name="group-product-sale"></div>';
                  htmlFinal +=
                    '<div class="buyxgety_item_title">Chỉ mua <span style="color:#315399;">' +
                    title +
                    "</span></div>";
                  htmlFinal += "</div>";
                  $(
              ...
              $("#add-to-cartQuickview")
                .removeClass("loading")
                .addClass("add-xy");
              if (is_xy_first !== 0 || count_unavailable > 0)
                $(".q-selector-buyxgety").removeClass("hidden");
            });
          } else {
            $("#add-to-cartQuickview").length > 0
	...
	onMainProductAdd: function (){

	},
    addCartBuyXGetY_detail: function (
      is_accept,
      variant_id,
      product_id,
      quantity,
      ...
      var dom_parent =
        buy_at.indexOf("normal") > -1
          ? ".selector-buyxgety #buyxgety-product-list"
          : ".q-selector-buyxgety #buyxgety-product-list";
      if (self.check_getRecommendeds) {
        var promotion_variant_checked = $(dom_parent).find(
          "input[name='group-product-sale'][type='radio']:checked"
        );
        var apply_variant = $(dom_parent)
          .find("input[name='group-product-sale'][type='radio']:checked")
          .data("applyvariant");
        var notGift = promotion_variant_checked.hasClass("not_gift");
        if (
          promotion_variant_checked &&
          promotion_variant_checked.length > 0 &&
          notGift == false
        ) {
          var promotion_countbuy = "",
            promotion_countgift = "",
            promotion_variant_id = "";

          promotion_countbuy = parseInt(
            promotion_variant_checked.attr("data-countbuy")
          );
          promotion_countgift = parseInt(
            promotion_variant_checked.attr("data-countgift")
          );
          ...
          if (is_valid_rule || is_accept) {
            jQuery.ajax({
              ...
              success: function (cart) {
                ...
                if (is_valid_rule) {
                  self.setPromotionStorage_cus(
                    variant_id,
                    promotion_countbuy,
                    promotion_countgift,
                    promotion_variant_id
                  );
                  var old_promotion_variant_id =
                    self.getPromotionStorage(variant_id);
                } else {
                  var old_promotion_variant_id =
                    self.getPromotionStorage(variant_id);
                  if (old_promotion_variant_id == undefined) {
                    ...
                    if (!old_promotion_variant_id[promotion_variant_id]) {
                      ...
                      old_promotion_variant_id[
                        promotion_variant_id
                      ].hasOwnProperty("priority") &&
                      old_promotion_variant_id[promotion_variant_id].priority ==
                        false
                    ) {
                      self.setPromotionStorage_cus(
                        variant_id,
                    ...
                    function (vIdGift, infoGift) {
                      var filterGiftInCart = cartGet.items.filter(
                        (x) =>
                          ...
                          x.promotionby[0].product_id == product_id
                      );
                      if (
                ...
                if (gOSP > 0) {
                  if (cart.quantity - gOSP == promotion_countbuy) {
                    ...
                    var promoteValid =
                      parseInt(
                        (cart.quantity - gOSP - promotion_countbuy) /
                          promotion_countbuy
                      ) * promotion_countgift;
                  } else {
                    var promoteValid = 0;
                  ...
                  var promoteValid =
                    parseInt(cart.quantity / promotion_countbuy) *
                    promotion_countgift;
                }
            ...
            var swalName =
              buy_at.indexOf("normal") > -1
                ...
                : "swal-alert-km-qv";
            var swalText =
              "Số lượng đặt sản phẩm <strong> " +
              prodTitle +
              " </strong> của bạn chưa đủ để được tặng/giảm sản phẩm";
    ...
    addCartBuyXGetY_itemLoop: function (
      product_id,
      variant_id,
      quantity,
      properties,
      ...
      if (
        ...
        (propertieskho == "" || propertieskho == "undefined")
      ) {
        ...
        var data_param = {
          quantity: quantity,
          id: variant_id,
          properties: { preorder: properties, kho: propertieskho },
        };
      } else if (propertieskho != "" || propertieskho != "undefined") {
        var data_param = {
          quantity: quantity,
          id: variant_id,
          properties: { kho: propertieskho },
        };
      } else {
        var data_param = {
          quantity: quantity,
          id: variant_id,
          properties: { preorder: properties },
        };
      }
      var params = {
        type: "POST",
        ...
        success: function (line_item) {
          if (
            ...
            buyXgetY.check_getRecommendeds = false;
            HaravanPromotion.GetRecommendeds(
              parseInt(product_id),
              function (result) {
                // success
                if (result.recommendeds.length > 0) {
                  buyXgetY.check_getRecommendeds = true;
                  $.each(result.recommendeds, function (i, v) {
                    var html = "<h5>Sản phẩm tặng kèm</h5>",
                      html2 = "";
                    html +=
                      '<div class="popup-content"><h4><span id="sanphamquatang">' +
                      v.product_name +
                      "</span></h4>";
                    html +=
                      '<div class="imgqua"><img class="imgnhanqua" src="' +
                      v.product_images[0] +
                      '" alt="' +
                      v.product_name +
                      '"/></div>';
                    if (v.is_apply_variant == true) {
                      ...
                      html +=
                        '<div class="variant_select"><p>Chọn size bạn mong muốn</p><select class="variant_gift">';
                      $.each(v.apply_variants, function (i2, v2) {
                      ...
                      html +=
                        '<div class="variant_select hidden"><p>Chọn size bạn mong muốn</p><select class="variant_gift">';
                      html +=
                        '<option value="' +
                        promotion_variant_id +
                        '" data-countbuy="' +
                    ...
                    html +=
                      '<a href="javascript:void(0)" class="btn_nhanqua" data-variantid="">BẤM VÀO ĐÂY ĐỂ NHẬN QUÀ</a>';
                    $("#nhanqua").html(html);
                  });
                  if (checkPromotionRecommend == true) {
                    ...
                    self.setPromotionStorage(
                      variant_id,
                      quantity,
                      promotion_quantity,
                      promotion_variant_id
                    );
                    self.AddCartItemPromotion(
                      promotion_variant_id,
                      promotion_quantity
                    );
                  }
              ...
              function () {
                // always
                if (
                  self.check_getRecommendeds &&
                  checkPromotionRecommend == true
                ) {
                  $(".btn_nhanqua").click(function () {
                    var promotion_variant_id = $(this)
                        .parents("#nhanqua")
                        .find(".variant_select .variant_gift")
                        .val(),
                      promotion_quantity = $(this)
                        .parents("#nhanqua")
                        .find(".variant_select .variant_gift option:selected")
                        .data("countgift");
                    self.AddCartItemPromotion(
                      promotion_variant_id,
                      promotion_quantity,
    ...
    UpdateCartFromCart: function () {
      ...
      for (var i = 0; i < listCart.length; i++) {
        ...
        var qty = 0;
        var variant_id = Number(
          $(listCart[i]).attr("id").replace("updates_", "")
        );
    ...
    setPromotionStorage: function (
      main_variant_id,
      main_quantity,
      apply_quantity,
      promotion_variant_id_raw,
      is_not_overwrite
    ) {
    ...
    setPromotionStorage_cus: function (
      main_variant_id,
      main_quantity,
      apply_quantity,
      promotion_variant_id_raw,
    ...
    checkPromotionRecommended: function (arr_product_id, callback) {
      if (hrvPromotionInited) {
        HaravanPromotion.CheckRecommendeds(
          arr_product_id,
          function (result) {
            // success
        ...
        this.checkPromotionRecommended(arr_prod_id, function (result) {
          $.each(result, function (i, item) {
            ...
            if (item.has_gift == true || item.has_discount == true) {
              $(
                elementGift + '[data-id="' + item.product_id + '"]'
              ).removeClass("hidden");
              $(elementGift)
                .parents(".product-item")
                .find(".product_action .btn_addtocart")
                .removeClass("loading");
            } else {
              $(elementGift)
                .parents(".product-item")
